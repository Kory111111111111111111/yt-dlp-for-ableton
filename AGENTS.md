...
- This repo is an Ableton Live extension (`YouTubeToAbleton`, author koryi, manifest v0.1.6) that downloads YouTube audio via yt-dlp/ffmpeg and imports WAV or MP3 clips into the Live set (`isWarped: false`).
...
- Key modules: `extension.ts` (`activate`, commands/UI/pipeline), `youtubeDownload.ts`, `youtubeUrl.ts` (watch/youtu.be/shorts/live/embed/music/m), `youtubeTooling.ts`, `toolsConfig.ts`, `tempCleanup.ts`, `bpmDetection.ts` (tempo detection via `music-tempo` + `audio-decode`), plus HTML dialog templates; tests in `tests/` via `tsx --test`.
